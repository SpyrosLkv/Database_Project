...
CREATE TABLE IF NOT EXISTS `semester_project`.`Loan` (
  ...
  INDEX `loan_date_idx` (`loan_date` ASC) VISIBLE,
...
SELECT T1.category AS category1, T2.category AS category2, COUNT(*) AS pair_count
        FROM (
            SELECT BI1.category_id AS category_id1, BI2.category_id AS category_id2
            FROM Belongs_in BI1
            INNER JOIN Belongs_in BI2 ON BI1.book_ISBN = BI2.book_ISBN AND BI1.category_id < BI2.category_id
            GROUP BY BI1.book_ISBN, BI1.category_id, BI2.category_id
            HAVING COUNT(*) = 1
        ) AS Pairs
        INNER JOIN Thematic_Category T1 ON Pairs.category_id1 = T1.category_id
        INNER JOIN Thematic_Category T2 ON Pairs.category_id2 = T2.category_id
